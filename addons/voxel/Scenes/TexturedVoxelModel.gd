...
# 256 x 16 x N, R8
@export var voxel_texture: ImageTexture3D:
	get:
...
func configure():
	...
	if Engine.is_editor_hint():
		if not is_valid_size_in_cubes:
			push_error("%s: Invalid model_size. Must be between (1, 1, 1) and (16, 16, 16)" % [name])
...
func set_shader_parameters():
	for shader_material in shader_materials:
		...
		shader_material.set_shader_parameter("voxel_texture", voxel_texture)
...
func expensive_verifications():
	...
	if not is_valid_model:
		push_error("%s: Invalid model_texture. Must be 256x16 FORMAT_RGB8" % [name])

	# [TODO: Recreate Checks]
	#var voxel_cube_0: Image = voxel_texture.get_layer_data(0)
	#var is_valid_voxel: bool = (
		#not voxel_cube_0.is_compressed() and
		#voxel_texture.get_width() == 256 and
		#voxel_texture.get_height() == 16
	#)
	#if not is_valid_voxel:
		#push_error("%s: Invalid voxel_textures. Must be 256x16xN FORMAT_R8" % [name])
	...
	if not is_valid_model:
		push_error("%s: Invalid palette_texture. Must be 6x256 FORMAT_RGBA8" % [name])

	return is_valid_model and is_valid_palette
